...
async function gotoCollaboration(page: Page) {
  ...
  await expect(page.locator("body")).toContainText(
    /collaborative|collaboration|workspace/i,
    { timeout: 30_000 },
  );
...
test.describe("Collaboration hub review journey", () => {
  ...
  test("CollaborationScreen renders with its header and tab strip", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /collaborative|collaboration/i }),
    ).toBeVisible();

    // The original tab strip should still be present after the merge.
    await expect(
      page.getByRole("button", { name: /^Overview\b/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /^Comments\b/i }),
    ).toBeVisible();
  });
  ...
  test("new telecom-aware review tabs (Annotations, Approval, Audit) are present", async ({
    ...
    await expect(
      page.getByRole("button", { name: /annotations/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /approval|validation|approbation/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /^Audit\b/i }),
    ).toBeVisible();
  });

  test("Annotations tab reveals the report-section annotations surface", async ({
    page,
  }) => {
    await gotoCollaboration(page);
    ...
    await expect(
      page.getByText(/report annotations|annotations/i).first(),
  ...
  test("Approval tab reveals the approval workflow with its REVIEW state", async ({
    page,
  }) => {
    await gotoCollaboration(page);
    ...
    await expect(
      page.getByText(/approval workflow|approval|validation/i).first(),
    ).toBeVisible({ timeout: 15_000 });
    ...
    await expect(page.locator("body")).toContainText(
      /review|approval workflow|submit for review/i,
    );
  });
  ...
  test("Audit tab reveals the audit trail surface", async ({ page }) => {
    ...
    await expect(
      page.getByText(/audit trail|audit/i).first(),
    ).toBeVisible({ timeout: 15_000 });
    await expect(page.locator("body")).toContainText(
      /audit trail|log of every action|audit/i,
    );
  });

  test("switching across the review tabs swaps their content", async ({
    page,
  }) => {
    await gotoCollaboration(page);
    ...
    await clickTab(page, /approval|validation|approbation/i);
    await expect(
      page.getByText(/approval workflow|approval/i).first(),
    ).toBeVisible({ timeout: 15_000 });
