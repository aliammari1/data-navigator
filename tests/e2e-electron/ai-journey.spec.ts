 ...
 *   3. AI Prompt Composer: textarea interaction, prompt staging
  ...
  test("complete AI user experience with screenshots", async () => {
    ...
    try {
      ...
      await test.step("navigate to Moudir AI chat screen", async () => {
        ...
        await expect(header.getByText("Moudir", { exact: true }).first()).toBeVisible();
        await expect(header.getByText(/IA locale/i)).toBeVisible();
      ...
      await test.step("open and inspect conversation search palette", async () => {
        ...
        if (await searchButton.isVisible()) {
          ...
          const paletteDialog = window.getByRole("dialog");
