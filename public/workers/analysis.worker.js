...
var require_DBSCAN = __commonJS({
  "node_modules/.pnpm/density-clustering@1.3.0/node_modules/density-clustering/lib/DBSCAN.js"(exports, module) {
    function DBSCAN2(dataset, epsilon, minPts, distanceFunction) {
      this.dataset = [];
      this.epsilon = 1;
      this.minPts = 2;
    ...
    DBSCAN2.prototype.run = function(dataset, epsilon, minPts, distanceFunction) {
      this._init(dataset, epsilon, minPts, distanceFunction);
    ...
    DBSCAN2.prototype._init = function(dataset, epsilon, minPts, distance) {
      if (dataset) {
...
var require_KMEANS = __commonJS({
  "node_modules/.pnpm/density-clustering@1.3.0/node_modules/density-clustering/lib/KMEANS.js"(exports, module) {
...
var require_PriorityQueue = __commonJS({
  "node_modules/.pnpm/density-clustering@1.3.0/node_modules/density-clustering/lib/PriorityQueue.js"(exports, module) {
...
var require_OPTICS = __commonJS({
  "node_modules/.pnpm/density-clustering@1.3.0/node_modules/density-clustering/lib/OPTICS.js"(exports, module) {
    if (typeof module !== "undefined" && module.exports) {
...
var require_lib = __commonJS({
  "node_modules/.pnpm/density-clustering@1.3.0/node_modules/density-clustering/lib/index.js"(exports, module) {
    if (typeof module !== "undefined" && module.exports) {
      module.exports = {
...
// src/platform/viz/seeded-rng.ts
var DEFAULT_SEED = 42;
...
// src/platform/viz/stats-core.ts
function logGamma(x) {
  const g = 7;
  const c = [
    0.9999999999998099,
...
function medianAbsoluteDeviation(xs, scale = 1.4826) {
  if (xs.length === 0) return 0;
  const med = median(xs);
...
// src/workers/analysis.worker.ts
var EPSILON = 1e-12;
async function kMeans(data, options) {
  const n = data.length;
...
async function dbscan(data, eps = 0.5, minPts = 5) {
  ...
  const engine = new import_density_clustering.DBSCAN();
  const clusters = engine.run(data, eps, minPts);
...
async function attribution(X, y) {
  const n = X.length;
  const d = X[0]?.length ?? 0;
  ...
  const Xm = new Matrix2(z);
  const svd = new SVD2(Xm, { autoTranspose: true });
  const u = svd.leftSingularVectors;
  const v = svd.rightSingularVectors;
  const s = svd.diagonal;
  const tol = (s.length ? Math.max(...s) : 0) * Math.max(n, d) * Number.EPSILON;
  const uty = u.transpose().mmul(Matrix2.columnVector(yz)).to1DArray();
  const scaled = uty.map((val, i) => s[i] > tol ? val / s[i] : 0);
  const coefficients = v.mmul(Matrix2.columnVector(scaled)).to1DArray();
...
async function correlationMatrix(data, columns = []) {
  ...
  if (nRows < 2 || nCols === 0) {
    return {
      matrix: Array.from(
        { length: nCols },
        (_, i) => Array.from({ length: nCols }, (_2, j) => i === j ? 1 : 0)
      ),
      columns: outCols
    };
  }
  ...
  const matrix2 = Array.from(
    { length: nCols },
    (_, i) => Array.from({ length: nCols }, (_2, j) => {
      if (i === j) return 1;
  ...
  return { matrix: matrix2, columns: outCols };
}
async function welchTTest(a, b, alpha = 0.05) {
  const na = a.length;
  const nb = b.length;
  if (na < 2 || nb < 2) {
    return {
...
async function anova1(values, factor, alpha = 0.05) {
...
async function detectAnomalies(values, options = {}) {
  ...
  if (method === "iqr") {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (v < lower || v > upper) {
        indices.push(i);
        scores.push(Math.round(Math.max(lower - v, v - upper) / iqr * 1e3) / 1e3);
      }
    }
  } else if (method === "mad") {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (score > threshold) {
        indices.push(i);
        scores.push(Math.round(score * 1e3) / 1e3);
      }
    }
  } else {
    const m = mean(values);
...
async function gesdAnomalies(values, options = {}) {
  const n = values.length;
  ...
  return {
    ...
    scores: chosen.map((c) => Math.round(c.score * 1e3) / 1e3)
  };
}
async function ewma(values, alpha = 0.3) {
...
async function stlDecompose(values, period) {
  const n = values.length;
  if (n === 0 || period < 2) {
...
async function pelt(values, options = {}) {
  const n = values.length;
  if (n < 4) return [];
  ...
  for (let t2 = minSize; t2 <= n; t2++) {
    ...
    for (const s of candidates) {
      ...
      const cost = F[s] + segCost(s, t2) + penalty;
      if (cost < bestCost) {
        bestCost = cost;
        bestS = s;
      }
    ...
    F[t2] = bestCost;
    cp[t2] = bestS;
    candidates = nextCandidates.filter((s) => F[s] + segCost(s, t2) <= F[t2] + EPSILON);
    candidates.push(t2);
  }
  const points = [];
  let t = n;
...
async function holtWinters(values, options = {}) {
  const n = values.length;
  ...
  if (period < 2 || n < 2 * period) {
    let level2 = values[0];
    let trend2 = n > 1 ? values[1] - values[0] : 0;
    const fitted2 = new Array(n);
    fitted2[0] = level2;
    for (let i = 1; i < n; i++) {
      const prevLevel = level2;
      level2 = alpha * values[i] + (1 - alpha) * (level2 + trend2);
      trend2 = beta * (level2 - prevLevel) + (1 - beta) * trend2;
      fitted2[i] = level2 + trend2;
    }
    const forecast2 = Array.from({ length: horizon }, (_, h) => level2 + (h + 1) * trend2);
    return { fitted: fitted2, forecast: forecast2 };
  }
