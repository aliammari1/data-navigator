  ...
  if (!loadPromise) {
    loadPromise = (async () => {
      ...
      try {
        ...
        throw new Error(`Pyodide CDN unreachable (${PYODIDE_CDN}pyodide.js). Check your network connection and try again. Underlying: ${err instanceof Error ? err.message : String(err)}`);
      }
      const loader = self.loadPyodide;
      if (!loader)
        throw new Error("Pyodide loader script imported but `loadPyodide` is undefined. Possible CDN content mismatch.");
