...
import { MODEL_CATALOG } from "@/lib/agent-canvas/types";
import { loadLLM } from "@/lib/agent-canvas/llm";
import { loadDelimitedCSVFromFile, loadJSONToDuckDB } from "@/lib/duckdb";
import { generateDataset, DEMO_PILLS } from "@/lib/agent-canvas/demo-data";
import type { DemoDataset } from "@/lib/agent-canvas/demo-data";
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        if (ext === "csv" || ext === "tsv" || ext === "txt") {
          ...
          setProgress(`Loading ${parsed.length.toLocaleString()} rows…`);
          await loadJSONToDuckDB(tableName, parsed);
        } else {
      ...
      {DEMO_PILLS.map((pill) => (
        <button
          ...
          className={cn(
            ...
            "bg-gradient-to-r border border-white/10 transition-all hover:scale-105 active:scale-95",
            "disabled:opacity-60 disabled:cursor-not-allowed",
            `bg-gradient-to-r ${pill.color}`,
          )}
        >
          {loading === pill.id ? (
