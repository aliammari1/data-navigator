...
import type { AgentThought, ThoughtKind } from "@/lib/agent-canvas/types";
...
export function ThoughtStream({ thoughts, className }: Props) {
  ...
  if (thoughts.length === 0) {
    return (
      <div
        className={cn(
          "flex items-center justify-center h-24 text-slate-600 text-xs",
          className,
        )}
      >
    ...
    <div className={cn("overflow-y-auto space-y-0.5 pr-1", className)}>
      {thoughts.map((t) => (
        <div key={t.id} className="flex gap-2 group">
          <span
            className={cn(
              "shrink-0 text-[10px] mt-[3px] select-none",
              KIND_STYLE[t.kind],
            )}
          >
            {KIND_PREFIX[t.kind]}
          ...
          <div className="min-w-0 flex-1">
            <span className="text-[10px] text-slate-600 mr-1.5 select-none">
              [{t.agent}]
            </span>
            <span
              className={cn(
