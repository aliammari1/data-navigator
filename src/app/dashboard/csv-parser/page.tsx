...
function detectType(values: unknown[]): ColType {
  ...
  for (const v of nonNull) {
    ...
    else if (/^\d{4}-\d{2}-\d{2}/.test(s) || /^\d{2}\/\d{2}\/\d{4}/.test(s)) dates++;
  }
...
function castValue(v: unknown, type: ColType): unknown {
  ...
  if (type === "boolean") return s === "true" || s === "1" || s === "yes" || s === "oui";
  if (type === "date") {
    const d = new Date(s);
...
function applyFilter(row: Record<string, unknown>, filter: string): boolean {
  ...
  try {
    ...
    if (eqMatch) {
      ...
      const na = Number(val), nb = Number(target);
      if (op === "=") return a === b;
      if (op === "!=") return a !== b;
    ...
    if (likeMatch) {
      ...
      return String(row[col] ?? "").toLowerCase().includes(sub.toLowerCase());
    }
    return true;
  } catch {
    return true;
...
export default function CSVParserPage() {
  ...
  const handleLoadDB = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Table "${safeName}" loaded — ${rows.length.toLocaleString()} rows`);
    } catch (e) {
      toast.error(`DuckDB error: ${String(e).slice(0, 80)}`);
    } finally {
      setLoadingDB(false);
    }
  }, [allTransformed, tableName, colConfigs, rawText, addDataset, setActiveDataset, markTableLoaded]);
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 flex-none border-r border-border flex flex-col">
          ...
          >
            ...
            {isDragActive && (
              <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-emerald-500/10 pointer-events-none">
                <p className="text-emerald-400 text-sm font-medium">Drop file here</p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 px-3 mt-2">
            ...
            <button
              type="button"
              onClick={() => { setRawText(""); setParsed(null); setColConfigs([]); setLoadedTable(null); }}
              disabled={!rawText}
          ...
          <div className="px-3 mt-3 space-y-2.5 pb-3 border-b border-border">
            ...
            <div className="space-y-1.5">
              {[
                { key: "header", label: "First row is header", val: hasHeader, set: setHasHeader },
                { key: "empty", label: "Skip empty lines", val: skipEmpty, set: setSkipEmpty },
                { key: "trim", label: "Trim whitespace", val: trimWS, set: setTrimWS },
          ...
          {parsed && (
            <div className="px-3 mt-3 pb-3 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  {loadingDB ? <Loader2 className="w-3 h-3 animate-spin" /> : <Database className="w-3 h-3" />}
                  Load
                </button>
              </div>
              {loadedTable && (
                <div className="flex items-center gap-2 px-2.5 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                  ...
                  <span className="text-[11px] text-emerald-300">Table <code className="font-mono">{loadedTable}</code> ready</span>
                </div>
              )}
              <button
                type="button"
              ...
              >
                <Download className="w-3 h-3" /> Export CSV ({allTransformed.rows.length.toLocaleString()} rows)
              </button>
            </div>
          )}
        </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1 max-w-xs">
                  Paste delimited text on the left, configure the delimiter, then click <strong>Parse</strong>.
                </p>
              </div>
              <div className="flex flex-wrap justify-center gap-2 text-xs text-muted-foreground">
                {["Rename columns", "Change types", "Filter rows", "Load to DuckDB"].map((f) => (
                  <span key={f} className="flex items-center gap-1 px-2.5 py-1 bg-muted border border-border rounded-lg">
            ...
            <>
              ...
              <div className="flex-none border-b border-border px-4 py-2 flex items-center gap-3">
                ...
                >
                  ...
                  {showColPanel ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                </button>
                ...
                >
                  ...
                  Filter
                  {filterExpr && <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />}
                    ...
                    <div className="p-3 overflow-x-auto">
                      <div className="flex gap-1.5 min-w-max">
                        ...
                        <div className="flex gap-1 mr-2">
                          <button
                            type="button"
                            onClick={() => setColConfigs((c) => c.map((x) => ({ ...x, include: true })))}
                          ...
                          <button
                            type="button"
                            onClick={() => setColConfigs((c) => c.map((x) => ({ ...x, include: false })))}
                            className="px-2 py-1 text-[10px] border border-border rounded-lg text-muted-foreground hover:bg-accent transition-colors"
                          >
                            None
                          </button>
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setColConfigs((prev) =>
                                    prev.map((c, j) => j === i ? { ...c, alias: e.target.value } : c),
                                  )
                                }
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setColConfigs((prev) =>
                                    prev.map((c, j) => j === i ? { ...c, include: !c.include } : c),
                                  )
                                }
                                className={cn(
                            ...
                            <div className="flex gap-1 flex-wrap">
                              {(["string", "number", "date", "boolean"] as ColType[]).map((t) => (
                                <button
                                  key={t}
                                  type="button"
                                  onClick={() =>
                                    setColConfigs((prev) =>
                                      prev.map((c, j) => j === i ? { ...c, type: t } : c),
                                    )
                                  }
                                  className={cn(
                                    "text-[9px] px-1.5 py-0.5 rounded font-mono transition-opacity",
                                    col.type === t ? "opacity-100" : "opacity-30 hover:opacity-60",
                                    t === "number" && "bg-emerald-500/15 text-emerald-400",
                                    t === "string" && "bg-blue-500/15 text-blue-400",
                                    t === "date" && "bg-purple-500/15 text-purple-400",
                                    t === "boolean" && "bg-amber-500/15 text-amber-400",
                                  )}
                                >
                                  {t}
                                </button>
                  ...
                  >
                    <div className="p-3 space-y-2">
                      ...
                      <p className="text-[10px] text-muted-foreground pl-5">
                        Syntax: <code className="font-mono">COL = value</code> · <code className="font-mono">COL &gt; 100</code> · <code className="font-mono">COL LIKE %text%</code> · <code className="font-mono">COL != value</code>
                      </p>
                    </div>
                  </motion.div>
                )}
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-1.5">
                                <span className="font-medium text-foreground text-[11px]">{h}</span>
                          ...
                          {transformed.headers.map((h) => {
                            ...
                            return (
                              <td
                                key={h}
                                className={cn(
                                  "px-3 py-1.5 border-r border-border/20 font-mono whitespace-nowrap max-w-[220px] overflow-hidden text-ellipsis text-[11px]",
                                  val === null ? "text-muted-foreground/40 italic" :
                                  cfg?.type === "number" ? "text-emerald-300 text-right" :
                                  cfg?.type === "date" ? "text-purple-300" :
                                  cfg?.type === "boolean" ? "text-amber-300" :
                                  "text-foreground",
                                )}
