...
const DEMO_COL_LINEAGE: ColumnLineage[] = [
  {
...
export default function LineagePage() {
  ...
  const colLineage = useMemo(() => {
    ...
    return {
      incoming: DEMO_COL_LINEAGE.filter(
        (c) => c.targetNode === selectedNode.id,
      ),
      outgoing: DEMO_COL_LINEAGE.filter(
        (c) => c.sourceNode === selectedNode.id,
      ),
    };
  }, [selectedNode]);
                    ...
                    return (
                      <div key={node.id} data-node="true">
                        <NodeCard
                          ...
                          highlighted={
                            h as "upstream" | "downstream" | "none" | "selected"
                          }
                          position={pos}
                          onClick={() => {
                            setSelectedNode(
              ...
              <div className="max-w-4xl mx-auto space-y-4">
                <div className="bg-card border border-border rounded-xl p-4">
                  <h2 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                    ...
                    Column-Level Lineage ({DEMO_COL_LINEAGE.length} tracked
                    mappings)
                  </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {["raw_sales", "clean_sales", "revenue_metrics"].map(
                    (nodeId) => {
                      const node = nodes.find((n) => n.id === nodeId);
                      if (!node) return null;
                      const colsIn = DEMO_COL_LINEAGE.filter(
                        (c) => c.targetNode === nodeId,
                      );
                      const colsOut = DEMO_COL_LINEAGE.filter(
