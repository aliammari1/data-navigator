...
// ─── Shared app Awareness (ONE roster) ────────────────────────────────────────
// The single Awareness bound to the app doc. Both the LAN transport
// (`@/platform/lan/lan-collab`) and room handles (`./room`) MUST use this
// instance — a second Awareness on the same Hocuspocus document makes every
// machine appear twice in every peer roster.

let _appAwareness: Awareness | null = null;

/** Lazily create the shared Awareness for the app doc (browser only, null under SSR). */
export function getAppAwareness(): Awareness | null {
  if (typeof window === "undefined") return null;
  if (!_appAwareness) _appAwareness = new Awareness(ydoc);
  return _appAwareness;
}
