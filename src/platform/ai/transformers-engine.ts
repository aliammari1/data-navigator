...
/**
 * Transformers.js LLM engine — DEBUG BUILD
 *
 * Search console for: [LLM]
 *
 * This file is intentionally noisy so you can see where the first agent stops.
 *
 * HOME: this is the canonical home for the @huggingface/transformers generative
 * engine. It lives under `src/platform/ai` so the heavy engine import stays
...
// Default OFF in production: the verbose `[LLM]` trace + safeStringify of every
// pipeline result is a measurable cost on the inference hot path. Opt back in
// explicitly via setLLMDebug(true) or per-call `debug: true`. Dev keeps it on.
let _debug = process.env.NODE_ENV !== "production";
let _sequence = 0;
    ...
    async () => {
      ...
      _loadPromise = span(
        ...
        async () => {
          ...
          log(
            "configuring offline transformers env",
            { allowRemoteModels, useBrowserCache },
            { traceId, debug },
          );

          // Pin offline runtime: local ORT wasm + local/cached weights. When
          // `allowRemoteModels` is undefined the persisted user preference
    ...
    async () => {
      ...
      const result = await span(
        ...
        async () => {
          ...
          try {
            // biome-ignore lint/style/noNonNullAssertion: guarded non-null above (throws if !_pipe); module-level `let` widens back to `| null` across awaits, and `_pipe?.(...)` would yield `undefined` and break the awaited inference type.
