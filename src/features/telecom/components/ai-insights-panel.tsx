...
export function AIInsightsPanel({
  ...
  const sevCfg = {
    critical: {
      ...
      bar: "bg-red-500",
      icon: (
        <AlertTriangle className="w-4 h-4 text-red-500 dark:text-red-400 flex-none mt-0.5" />
      ),
    },
    warning: {
      ...
      bar: "bg-amber-500",
      icon: (
        <AlertCircle className="w-4 h-4 text-amber-500 dark:text-amber-400 flex-none mt-0.5" />
      ),
    },
    info: {
      ...
      bar: "bg-blue-500",
      icon: (
        <Info className="w-4 h-4 text-blue-500 dark:text-blue-400 flex-none mt-0.5" />
      ),
    },
    positive: {
    ...
    return (
      <div className="flex flex-col items-center gap-3 py-12 text-muted-foreground">
        ...
        <div className="text-sm">
          Aucune anomalie détectée — toutes les métriques sont dans la plage
          normale.
        </div>
      </div>
    );
      ...
      {insights.map((ins, idx) => {
        ...
        return (
          <motion.div
            ...
            className={cn(
              "rounded-xl border overflow-hidden flex",
              cfg.border,
              cfg.bg,
            )}
          >
