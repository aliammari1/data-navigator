 ...
 * Spatial canvas for data exploration. No tabs. No sidebars.
 * Everything lives on the canvas: charts, insights, tables, agent reasoning.
 *
 * Properly wired:
...
export default function WorkbenchScreen() {
  // ── Data ───────────────────────────────────────────────────────────────────
  const [tableName, setTableName] = useState(TELECOM_TABLE_BASE);
  ...
  // ── Workbench Store ────────────────────────────────────────────────────────
  const addCard = useWorkbenchStore((s) => s.addCard);
  const clearCanvas = useWorkbenchStore((s) => s.clearCanvas);
  const setAgentRunning = useWorkbenchStore((s) => s.setAgentRunning);
  const setAgentTrace = useWorkbenchStore((s) => s.setAgentTrace);
  const setAgentStatusText = useWorkbenchStore((s) => s.setAgentStatusText);
  const managerAnswer = useWorkbenchStore((s) => s.managerAnswer);
  const setManagerAnswer = useWorkbenchStore((s) => s.setManagerAnswer);
  const activeIntent = useWorkbenchStore((s) => s.activeIntent);
  const setActiveIntent = useWorkbenchStore((s) => s.setActiveIntent);
  const addHistoryNode = useWorkbenchStore((s) => s.addHistoryNode);
  const attachCard = useWorkbenchStore((s) =>
    s.cards.find((c) => c.id === attachCardId),
  );
  ...
  // so they become visible rather than gathering at pixel (0,0).
  const updateCardRef = useWorkbenchStore((s) => s.updateCard);
  useEffect(() => {
    const cards = useWorkbenchStore.getState().cards;
    const needsFix = cards.filter((c) => c.w === 0 || c.h === 0);
    if (needsFix.length === 0) return;
    // Default sizes keyed by card type
    ...
    // Re-use nextPosition from the store so we don't overlap existing cards
    let cursorX = 40,
      cursorY = 40,
      nextRowMaxY = 40,
      screenW = 1200,
      margin = 24;
    // Account for valid (non-stale) cards that already have a position
    for (const c of cards) {
      if (c.w > 0 && c.h > 0) {
        cursorX = Math.max(cursorX, c.x + c.w + margin);
        nextRowMaxY = Math.max(nextRowMaxY, c.y + c.h + margin);
        if (cursorX > screenW) {
          cursorX = 40;
          cursorY = nextRowMaxY;
        }
    ...
    const updateStale = (index: number, list: typeof cards) => {
      ...
      updateCardRef(card.id, { x: cursorX, y: cursorY, w: size.w, h: size.h });
      cursorX += size.w + margin;
      if (cursorX > screenW) {
        cursorX = 40;
        cursorY += size.h + margin;
      }
      // Schedule next card in next animation frame to avoid stacking
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const resolvedTable = tables.includes(tableName)
          ? tableName
          : tables.includes(TELECOM_TABLE_BASE)
            ? TELECOM_TABLE_BASE
            : tables[0];
        ...
        if (!resolvedTable) {
          ...
          setSchema("");
          setDataError(
            "No DuckDB tables are loaded yet. Import data first, then return to the workbench.",
          );
          return;
        }
        ...
        const schemaText = normalizedColumns
          .map((c) => `${c.name} ${c.dbType}`)
          .join(", ");
      ...
      if (!columns.length) {
        setManagerAnswer(
          createManagerAnswer({
            ...
            summary:
              "No active table is loaded. Import a CSV or telecom report, then ask for a KPI, dashboard, investigation, signal scan, scenario, or brief.",
            assumptions: ["Moudir AI needs a loaded table before analysis."],
            evidence: [],
            followUps: ["Import a CSV", "Open a telecom report"],
            confidence: "high",
            status: "needs-input",
            language: "auto",
          }),
            ...
            if (results.length > 0) {
              ...
              setManagerAnswer(
                createManagerAnswer({
                  ...
                  summary: `I found ${results.length} matching row${results.length === 1 ? "" : "s"} and added one table card to the canvas.`,
        ...
        if (intent === "dashboard") {
          ...
          setManagerAnswer(
            createManagerAnswer({
              intent: "dashboard",
              title: spec.title,
              summary: `I created ${added} AI-designed dashboard card${added === 1 ? "" : "s"} on the canvas.`,
          ...
          setManagerAnswer(
            createManagerAnswer({
              ...
              assumptions: [`Model: ${gate.selectedModel}`],
              evidence: result.steps.map((s) => `${s.dimension}: ${s.reason}`),
              followUps: [
  ...
  const handleAutoDashboard = useCallback(async () => {
    ...
    if (!columns.length) {
      setManagerAnswer(
        createManagerAnswer({
          intent: "setup",
          title: "Import data first",
          summary: "AI Dashboard needs an active table before it can run.",
    ...
    try {
      const { spec, added } = await generateDashboardFromPrompt(
        `Create a comprehensive dashboard for ${tableName}`,
        gate.selectedModel,
        gate.host,
      );
      setManagerAnswer(
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="h-[calc(100vh-4rem)] relative overflow-hidden bg-background">
      {/* Header */}
      <div className="absolute top-0 left-0 right-0 z-40 flex items-center justify-between gap-3 px-4 py-2.5 border-b border-white/5 bg-background/80 backdrop-blur-sm">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="hidden min-w-0 items-center gap-1.5 rounded-lg border border-white/5 bg-white/5 px-2 py-1 text-[10px] text-muted-foreground sm:flex">
            ...
            <span className="max-w-32 truncate">{tableName}</span>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            type="button"
            onClick={clearCanvas}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] text-muted-foreground bg-white/5 hover:bg-white/10 border border-white/10 transition-all"
          >
