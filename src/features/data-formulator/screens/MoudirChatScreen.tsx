...
export default function MoudirChatScreen() {
    ...
    const conversationIds = useMoudirChatStore(
        useShallow((s) => s.conversations.map((c) => c.id)),
    );
    ...
    const [canvasOrientation, setCanvasOrientation] = usePersistentState<
        "horizontal" | "vertical"
    >(CANVAS_ORIENTATION_KEY, "horizontal");
    ...
    const askFlow = useCallback(
        async (prompt: string) => {
            ...
            await useMoudirChatStore
                .getState()
                .send(text, { datasetId: activeDatasetId ?? null });
        },
        [activeDatasetId],
    );
            ...
            void (async () => {
                ...
                useMoudirChatStore.setState({
                    ...
                    pendingScrollToMessageId:
                        detail.messageId == null ? null : String(detail.messageId),
                });
            })();
        };
        ...
        "moudir-chat",
        {
            ...
            "export-deck": async () => {
                ...
                toast("Génération de la présentation en cours…", {
                    description: "Deck exécutif 16:9 au standard McKinsey / BCG avec indicateurs et conclusions.",
                });
                try {
