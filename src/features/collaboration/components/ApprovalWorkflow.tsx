...
export function ApprovalWorkflow() {
  ...
  // ── Permission gates ────────────────────────────────────────────────────
  // 1) General edit permission (same pattern as CollaborationScreen): no
  //    dedicated "canApprove" flag exists, so canEditComments is the closest
  //    fit — it's already the flag every other collaborative mutation in this
  //    feature (comments/changes/chat) is gated behind.
  // 2) LAN session mutation check: a device's own stored role can be capped
  //    by a read-only LAN session grant (reviewer/viewer). `access.role` /
  //    `canEditComments` already factors this in via capRoleBySession, but we
  //    also check `canMutateLAN` directly against the live session role so a
  //    read-only guest can never approve regardless of how the device role
  //    resolves — belt-and-suspenders against the two gates drifting apart.
  ...
  const handleReturnToDraft = () => {
    if (!canTransition) return;
    transition("DRAFT", username, "returned to draft");
  };
  ...
  const handleApprove = () => {
    if (!canTransition) return;
    transition("APPROVED", username, comment);
  };
  const handleRequestChanges = () => {
    if (!canTransition || !comment.trim()) return;
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!canTransition || !comment.trim()}
              >
            ...
            {!canTransition && (
              <p className="text-[11px] text-muted-foreground">
                {canEdit
                  ? "Your LAN session is read-only — approving requires host or editor access."
                  : "You don't have permission to approve, reject, or request changes."}
              </p>
            )}
            {canTransition && !comment.trim() && (
              <p className="text-[11px] text-muted-foreground">
