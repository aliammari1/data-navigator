    ...
    <>
      ...
      <Section title="Privacy & Data" icon={Eye}>
        <div className="space-y-3 text-sm text-muted-foreground">
          <p>
            <strong className="text-foreground">100% offline</strong> — all data
            stays on this device. No telemetry, no cloud sync, no external API
            calls.
          </p>
          <p>
            <strong className="text-foreground">DuckDB</strong> runs locally
            (native on desktop, WebAssembly on the web build) and never uploads
            your data.
          </p>
          <p>
            <strong className="text-foreground">AI features</strong> use local
            algorithms and models cached on-device — your data never leaves the
            machine.
          </p>
          <p>
            <strong className="text-foreground">Settings</strong> are persisted
            durably in a local SQLite table and mirrored to localStorage for
            instant load. Use the Storage panel to back them up.
          </p>
        </div>
      </Section>
    </>
