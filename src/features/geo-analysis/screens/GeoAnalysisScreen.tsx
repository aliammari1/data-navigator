    ...
    <Card>
      <CardContent className="pt-8 pb-8">
        <div className="mx-auto max-w-md text-center text-sm text-muted-foreground">
          ...
          <p>
            Import a dataset with a region column (the telecom region mapping
            defaults to <code>ACCOUNT_GROUP_ID</code>) to populate the map,
            channel heatmap, and flows. Every figure here is computed directly
            from the active dataset in DuckDB.
          </p>
        </div>
      </CardContent>
    </Card>
        ...
        <Card size="sm">
          <CardContent className="pt-3">
            ...
            <div className="text-2xl font-bold">
              {fmtCompact(geo.totalTransactions)}
            </div>
          </CardContent>
        </Card>
        <Card size="sm">
        ...
        <div className="min-w-0 flex-1">
            ...
            <CardContent className="relative overflow-hidden rounded-xl p-0">
              {geo.mappedRegions.length > 0 ? (
                ...
                <div className="flex h-[500px] items-center justify-center p-6 text-center text-sm text-muted-foreground">
                  None of the {geo.regions.length} regions matched a known
                  governorate, so they cannot be placed on the map. They remain
                  listed on the right and in the channel heatmap.
                </div>
              )}
          ...
          {unmapped > 0 && (
            <p className="mt-2 text-xs text-muted-foreground">
              {unmapped} region{unmapped !== 1 ? "s" : ""} could not be matched to
              a known governorate and {unmapped !== 1 ? "are" : "is"} shown in the
              list only.
            </p>
          )}
        </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-xl font-bold">
                      {fmtN(selectedRegion.transactions)}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground">Revenue</div>
                    <div className="text-xl font-bold">
                      {fmtAmount(selectedRegion.revenue)}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground">
...
function FlowsTab({ geo }: { geo: ReturnType<typeof useGeoData> }) {
  ...
  const filtered = useMemo(() => {
    let rows: GeoFlow[] = channel
      ? geo.flows.filter((f) => f.channel === channel)
      : geo.flows;
    rows = [...rows].sort((a, b) =>
      sort === "volume"
    ...
    return (
      <Card>
        <CardContent className="pt-8 pb-8 text-center text-sm text-muted-foreground">
          Not enough channel/region detail in this dataset to build a
      ...
      <div className="flex gap-4">
        ...
        <div className="w-72 shrink-0 space-y-3">
          ...
          {selectedRegionIdx !== null &&
            selectedRegionIdx < geo.matrix.regions.length && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    <span className="truncate">
                      {geo.matrix.regions[selectedRegionIdx]}
                    </span>
                    <button
                      type="button"
                      className="text-lg leading-none text-muted-foreground hover:text-foreground"
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Regional transaction distribution, channel → region flows, and channel
            mix — all computed from the active dataset in DuckDB, fully offline.
          </p>
        </div>
