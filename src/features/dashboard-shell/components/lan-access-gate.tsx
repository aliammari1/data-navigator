...
export function LanAccessGate({
  ...
  // Prefill the join form from ?room=&code=&url= query params — the payload
  // makeJoinHttpUrl (lan-collab.ts) puts in join links / QR codes. Prefill
  // only, never auto-connect: the code may be stale, so the guest reviews and
  // clicks Connect. Runs once on mount (ref guard) so it never fights the
  // user's subsequent typing, then strips the params from the address bar so
  // the pairing code doesn't linger in the visible URL/history.
  // biome-ignore lint/correctness/useExhaustiveDependencies: consume join-link params once on mount
  useEffect(() => {
    ...
    const hasJoinParams =
      room !== null || code !== null || rawUrl !== null || params.get("role") !== null;
    if (!hasJoinParams) return;
    const patch: Partial<typeof settings> = {};
    if (room) patch.room = room;
    if (code) patch.pairingCode = code;
    if (rawUrl) {
      try {
        const parsed = new URL(rawUrl);
        if (parsed.protocol === "ws:" || parsed.protocol === "wss:") patch.url = rawUrl;
      } catch {
        // Not a parseable URL — ignore silently.
      }
    }
    if (Object.keys(patch).length > 0) persist(patch);
    for (const key of ["room", "code", "role", "url"]) params.delete(key);
