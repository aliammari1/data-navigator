...
export const HostMode: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(
      canvas.getByRole("button", { name: /start a session/i }),
    );
    await expect(
      canvas.getByText(/start the server on this computer/i),
    ).toBeInTheDocument();
  },
};
...
export const JoinMode: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.click(
      canvas.getByRole("button", { name: /join a session/i }),
    );
    await expect(
      canvas.getByText(/ask the host for the qr code/i),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /join session/i }),
    ).toBeInTheDocument();
  },
};
...
export const TogglesAdvanced: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("option", { name: /host — full control/i }),
    ).toBeInTheDocument();
  },
};
