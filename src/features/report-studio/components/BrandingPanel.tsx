...
function BrandingPanelImpl({ branding, update, setLogoFromFile, clearLogo }: BrandingPanelProps) {
  const fileRef = useRef<HTMLInputElement>(null)
  const logoUrl = useLogoObjectUrl(branding.logoBytes, branding.logoMime)
  const initials = useMemo(() => (branding.companyName || 'TX').slice(0, 2).toUpperCase(), [branding.companyName])
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Customize the appearance of all exported reports. Saved on-device.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Primary Color</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Company Logo</label>
            <div className="flex items-center gap-2">
              <input
                ref={fileRef}
                type="file"
              ...
              {branding.logoName && <span className="text-xs text-muted-foreground truncate">{branding.logoName}</span>}
            </div>
            <p className="text-xs text-muted-foreground">Stored locally and embedded into exported files — no network required.</p>
          </div>

          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Footer Text</label>
            <Input
          ...
          <label className="flex items-center gap-3 cursor-pointer py-2">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">Company name, color, logo and footer appear in all generated files</div>
            </div>
          </label>

          <div className="flex items-center gap-2 rounded-lg bg-green-500/10 border border-green-500/20 px-3 py-2">
            <span className="text-green-400 text-sm">✓</span>
            <span className="text-xs text-green-400">Settings saved automatically to local IndexedDB storage</span>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="rounded-xl overflow-hidden border border-border">
            <div className="p-5" style={{ backgroundColor: branding.primaryColor }}>
              <div className="flex items-center gap-3">
                {logoUrl ? (
                  ...
                  <img src={logoUrl} alt="logo" className="w-10 h-10 rounded object-contain bg-white/10 p-1" />
                ) : (
            ...
            <div className="bg-card p-5 space-y-3">
              <div className="h-3 rounded" style={{ backgroundColor: branding.primaryColor, opacity: 0.3, width: '60%' }} />
              <div className="h-2 bg-muted rounded w-full" />
              <div className="h-2 bg-muted rounded w-4/5" />
              <div className="grid grid-cols-3 gap-2 mt-3">
                {[0, 1, 2].map((i) => (
                  <div key={i} className="h-12 rounded-lg" style={{ backgroundColor: branding.primaryColor, opacity: 0.15 }} />
                ))}
              </div>
            </div>
...
export const BrandingPanel = memo(BrandingPanelImpl)
