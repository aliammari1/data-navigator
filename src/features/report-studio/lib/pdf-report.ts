...
export async function buildPdf(
  ...
  const primary =
    data.primaryColor && /^#[0-9a-fA-F]{6}$/.test(data.primaryColor)
      ? data.primaryColor
      : "#003087";
  ...
  content.push(
    { text: "Channel Performance", style: "h1", pageBreak: "before", margin: [0, 0, 0, 6] },
    {
      table: {
        ...
        body: [
          ["Channel", "Volume", "Success", "Revenue", "Status"].map((h) => ({
            text: h,
            style: "th",
          })),
          ...data.topChannels.map((ch) => [
            ...
            { text: fmtNum(ch.volume), style: "tdNum" },
            {
              ...
              style: "tdNum",
              color: statusColor(ch.successRate),
              bold: true,
            },
        ...
        table: {
          ...
          body: [
            ...
            [
              ...
              fmtNum(Math.round(c.prev.totalTransactions)),
            ],
  ...
  const docDefinition = {
      ...
      currentPage === 1
        ...
        : {
            columns: [
              ...
              {
                text: `Daily Transaction Report — ${data.date}`,
                fontSize: 8,
                color: "#94a3b8",
                alignment: "right",
                margin: [0, 20, 40, 0],
    ...
    footer: (currentPage: number, pageCount: number) => ({
      columns: [
        {
          text: data.footerText || "CONFIDENTIAL",
          fontSize: 8,
          color: "#94a3b8",
          margin: [40, 0, 0, 0],
        },
...
function kpiBlock(label: string, value: string, color: string): any {
  return {
    table: {
      widths: ["*"],
      body: [
        [{ text: value, color, bold: true, fontSize: 13, alignment: "center" }],
        [{ text: label, color: "#64748b", fontSize: 8, alignment: "center" }],
      ],
    },
    layout: "noBorders",
  };
