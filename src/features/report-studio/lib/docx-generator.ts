...
export async function buildDocx(
  ...
  sections.push(
    new Paragraph({
      children: [
        new TextRun({ text: company, bold: true, size: 48, color: primary, font: "Calibri" }),
      ],
  ...
  if (options.includeSections.executiveSummary) {
    ...
    if (data.aiNarrative?.executiveSummary) {
      sections.push(
        new Paragraph({
          children: [
            new TextRun({ text: data.aiNarrative.executiveSummary, font: "Calibri", size: 22 }),
          ],
          spacing: { after: 200 },
        }),
      );
        ...
        new Paragraph({
          children: [
            ...
            new TextRun({
              ...
              bold: true,
              color:
                data.successRate >= 95 ? "00AA44" : data.successRate >= 80 ? "FF9900" : "CC3300",
              font: "Calibri",
              size: 22,
            }),
            new TextRun({
  ...
  if (options.includeSections.keyMetrics) {
    sections.push(
      new Paragraph({
        text: "2. Key Performance Metrics",
        heading: HeadingLevel.HEADING_1,
        spacing: { before: 400, after: 200 },
      }),
    );
    ...
    const metricsRows: string[][] = cmp
      ? [
          ...
          [
            ...
            fmtNum(Math.round(cmp.prev.totalTransactions)),
            signedPct(pct(data.totalTransactions, cmp.prev.totalTransactions)),
          ],
          [
            "Success Rate",
            fmtPct(data.successRate),
            fmtPct(cmp.prev.successRate),
            `${(data.successRate - cmp.prev.successRate).toFixed(1)}pp`,
          ],
          [
            "Failed Transactions",
            fmtNum(data.failedTransactions),
            fmtNum(Math.round(cmp.prev.failedTransactions)),
            signedPct(pct(data.failedTransactions, cmp.prev.failedTransactions)),
          ],
          [
            "Total Revenue",
            fmtAmount(data.totalRevenue),
            fmtAmount(cmp.prev.totalRevenue),
            signedPct(pct(data.totalRevenue, cmp.prev.totalRevenue)),
          ],
        ]
      : [
          ["Metric", "Value"],
  ...
  if (options.includeSections.issues) {
    ...
    if (data.successRate < 90) {
      issues.push({
        severity: "[CRITICAL]",
        text: `Overall success rate ${fmtPct(data.successRate)} is below the 90% minimum threshold. Immediate escalation required.`,
      });
    }
    for (const c of data.topChannels.filter((c) => c.successRate < 80)) {
      issues.push({
        severity: "[WARNING]",
        text: `Channel "${c.name}" success rate ${fmtPct(c.successRate)} is critically low. Investigation required.`,
      });
    }
    ...
    if (issues.length === 0) {
      issues.push({
        severity: "[INFO]",
        text: "No critical issues or statistical anomalies detected this period. All channels are within acceptable parameters.",
      });
    }
        ...
        new Paragraph({
          ...
          children: [
            new TextRun({
              text: `${issue.severity} `,
              bold: true,
              color: issue.severity.includes("CRITICAL")
                ? "CC3300"
                : issue.severity.includes("WARNING")
                  ? "FF9900"
                  : issue.severity.includes("ANOMALY")
                    ? "9333EA"
                    : "0066CC",
              font: "Calibri",
              size: 22,
