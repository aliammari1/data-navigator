...
export async function buildPptx(
  ...
  // ── Slide 2: Executive Summary ─────────────────────────────────────────────
  {
    ...
    const kpis = [
      { label: 'Total Transactions', value: fmtNum(data.totalTransactions), color: '0066CC', icon: '#' },
      ...
      { label: 'Failed Transactions', value: fmtNum(data.failedTransactions), color: 'CC3300', icon: '!' },
    ]
    ...
    kpis.forEach((kpi, i) => {
      ...
      slide.addText(kpi.icon, {
        ...
        color: kpi.color, fontSize: 18, bold: true,
      })
      slide.addText(kpi.value, {
        x, y: y + 0.7, w: 4.4, h: 0.9,
        color: kpi.color, fontSize: 28, bold: true, align: 'center',
      })
      slide.addText(kpi.label, {
        x, y: y + 1.65, w: 4.4, h: 0.4,
        color: colors.subtext, fontSize: 12, align: 'center',
      })
    })
  }
  ...
  // ── Slide 3: Channel Performance Table ─────────────────────────────────────
  {
    ...
    slide.addText('Channel Performance', {
      ...
      color: colors.text, fontSize: 24, bold: true,
    })

    const tableData = [
      [
        { text: 'Channel', options: { bold: true, color: 'FFFFFF', fill: { color: colors.tableHeader } } },
        { text: 'Volume', options: { bold: true, color: 'FFFFFF', fill: { color: colors.tableHeader } } },
        { text: 'Success Rate', options: { bold: true, color: 'FFFFFF', fill: { color: colors.tableHeader } } },
        { text: 'Revenue', options: { bold: true, color: 'FFFFFF', fill: { color: colors.tableHeader } } },
      ],
      ...channels.slice(0, 8).map((ch, i) => [
        { text: ch.name, options: { fill: { color: i % 2 === 0 ? colors.tableRow1 : colors.tableRow2 }, color: template === 'clean-white' ? '1E293B' : 'E2E8F0' } },
        { text: fmtNum(ch.volume), options: { fill: { color: i % 2 === 0 ? colors.tableRow1 : colors.tableRow2 }, color: template === 'clean-white' ? '1E293B' : 'E2E8F0' } },
        { text: fmtPct(ch.successRate), options: { fill: { color: i % 2 === 0 ? colors.tableRow1 : colors.tableRow2 }, color: ch.successRate >= 95 ? '00AA44' : ch.successRate >= 80 ? 'FF9900' : 'CC3300', bold: true } },
        { text: fmtAmount(ch.revenue), options: { fill: { color: i % 2 === 0 ? colors.tableRow1 : colors.tableRow2 }, color: template === 'clean-white' ? '1E293B' : 'E2E8F0' } },
      ]),
    ]
  ...
  // ── Slide 6: Hourly Distribution ────────────────────────────────────────────
  {
    ...
    if (chartUri) {
      ...
      slide.addText('No hourly transaction data available for this period.', {
        x: 0.4, y: 3.4, w: 9.2, h: 0.6,
        color: colors.subtext, fontSize: 14, align: 'center', italic: true,
      })
    } else {
      slide.addChart('line', [
        {
  ...
  // ── Slide 7: Top Performing Channels ───────────────────────────────────────
  {
    ...
    top3.forEach((ch, i) => {
      ...
      slide.addText(medals[i], {
        x: 0.6, y: y + 0.35, w: 1.0, h: 0.8,
        color: medalColors[i], fontSize: 24,
      })
      slide.addText(`${i + 1}. ${ch.name}`, {
        x: 1.7, y: y + 0.1, w: 5.0, h: 0.6,
        color: colors.text, fontSize: 18, bold: true,
      })
      slide.addText(`Volume: ${fmtNum(ch.volume)}  |  Revenue: ${fmtAmount(ch.revenue)}`, {
        x: 1.7, y: y + 0.75, w: 5.0, h: 0.4,
        color: colors.subtext, fontSize: 11,
      })
      slide.addText(fmtPct(ch.successRate), {
        x: 7.0, y: y + 0.3, w: 2.3, h: 0.9,
        color: '00AA44', fontSize: 28, bold: true, align: 'right',
      })
    })
  }
  ...
  // ── Slide 8: Issues & Anomalies ─────────────────────────────────────────────
  {
    ...
    if (data.successRate < 90) {
      issues.push({ severity: 'CRITICAL', text: `Overall success rate ${fmtPct(data.successRate)} is below 90% threshold`, color: 'CC3300' })
    }
    if (failedPct > 10) {
      issues.push({ severity: 'WARNING', text: `High failure rate detected: ${fmtPct(failedPct)} of transactions failed`, color: 'FF9900' })
    }
    channels.filter((c) => c.successRate < 80).forEach((c) => {
      issues.push({ severity: 'WARNING', text: `${c.name}: Low success rate ${fmtPct(c.successRate)}`, color: 'FF9900' })
    })
    if (issues.length === 0) {
      issues.push({ severity: 'INFO', text: 'No critical issues detected. All channels operating within normal parameters.', color: '00AA44' })
    }
    ...
    issues.slice(0, 5).forEach((issue, i) => {
      ...
      slide.addText(issue.severity, {
        x: 0.7, y: y + 0.1, w: 1.4, h: 0.35,
        color: issue.color, fontSize: 10, bold: true,
      })
      slide.addText(issue.text, {
        x: 0.7, y: y + 0.42, w: 8.7, h: 0.38,
  ...
  // ── Slide 9: Trends vs Previous Period ─────────────────────────────────────
  {
    ...
    const comparisons = [
      { label: 'Transactions', current: data.totalTransactions, prev: cmp.prev.totalTransactions, unit: '' },
      ...
      { label: 'Failed Tx', current: data.failedTransactions, prev: cmp.prev.failedTransactions, unit: '' },
    ]
    ...
    comparisons.forEach((comp, i) => {
      ...
      slide.addText(comp.label, {
        x, y: y + 0.15, w: 4.4, h: 0.4,
        color: colors.subtext, fontSize: 12, align: 'center',
      })
      slide.addText(comp.unit === '%' ? fmtPct(comp.current) : fmtNum(Math.round(comp.current)), {
        x, y: y + 0.6, w: 4.4, h: 0.8,
        color: colors.text, fontSize: 26, bold: true, align: 'center',
      })
      slide.addText(`${arrow} ${Math.abs(change).toFixed(1)}% vs prev`, {
        x, y: y + 1.5, w: 4.4, h: 0.5,
        color: changeColor, fontSize: 13, align: 'center', bold: true,
      })
    })
    }
  }
  ...
  // ── Slide 10: Recommendations ──────────────────────────────────────────────
  {
    ...
    if (data.aiNarrative?.recommendations?.length) {
      ...
      if (lowChannels.length > 0) {
        recs.push(`Investigate and remediate low success rates on: ${lowChannels.map((c) => c.name).join(', ')}`)
      }
      recs.push('Schedule infrastructure review during low-traffic hours (02:00-05:00) to address performance bottlenecks')
      recs.push('Implement real-time alerting for success rate drops below 95% threshold on critical channels')
      ...
      recs.push('Review error code patterns and implement targeted fixes for top 3 recurring errors')
    }
    ...
    recs.slice(0, 6).forEach((rec, i) => {
      ...
      slide.addText(`${i + 1}`, {
        x: 0.4, y: y + 0.1, w: 0.55, h: 0.55,
        color: 'FFFFFF', fontSize: 14, bold: true, align: 'center', valign: 'middle',
      })
      slide.addText(rec, {
        x: 1.1, y: y + 0.05, w: 8.4, h: 0.65,
    ...
    slide.addText(`Generated: ${new Date().toLocaleString()} | ${company}`, {
      x: 0.4, y: 7.1, w: 9.2, h: 0.3,
      color: colors.subtext, fontSize: 9, align: 'center',
    })
  }
