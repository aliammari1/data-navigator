...
export async function buildXlsx(data: ReportData): Promise<ArrayBuffer> {
  ...
  if (data.hourlyData.length) {
    ...
    for (const h of data.hourlyData) {
      hourly
        .addRow({ hour: `${h.hour}:00`, count: Math.round(h.count), successRate: Number(h.successRate.toFixed(2)) })
        .commit()
    }
  }
  ...
  if (data.anomalies?.length) {
    ...
    for (const a of data.anomalies) {
      anom.addRow({ hour: `${a.hour}:00`, count: Math.round(a.count), successRate: Number(a.successRate.toFixed(2)), score: Number(a.score.toFixed(3)) }).commit()
    }
  }
