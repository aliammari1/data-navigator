  ...
  return (
    <div className="flex items-end gap-px h-4 mt-1.5">
      {histogram.slice(0, 30).map((b) => (
        <div
          key={b.bucket}
          className="w-1 bg-[var(--atlas-accent)] opacity-70 rounded-sm"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5">
      {profiles.map((p) => {
        ...
        return (
          <div
            key={p.name}
            className="rounded-[var(--atlas-radius-3)] border border-[var(--atlas-border)] bg-[var(--atlas-surface)] p-3 hover:border-[var(--atlas-accent-border)] transition-colors"
          >
            <div className="flex items-center justify-between gap-2 mb-1.5">
              <span className="text-[13px] font-semibold text-[var(--atlas-text)] truncate">
                {p.name}
              </span>
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]">
              <div className="text-[var(--atlas-text-subtle)]">distinct</div>
              <div className="text-right text-[var(--atlas-text)] tabular-nums">
                {fmt(p.cardinality)}
              </div>
              <div className="text-[var(--atlas-text-subtle)]">null %</div>
              <div
              ...
              {p.semantic === "numeric" && (
                <>
                  <div className="text-[var(--atlas-text-subtle)]">range</div>
                  <div className="text-right text-[var(--atlas-text)] tabular-nums">
                    {fmt(Number(p.min))} → {fmt(Number(p.max))}
                  </div>
                  {p.avg !== undefined && (
                    <>
                      <div className="text-[var(--atlas-text-subtle)]">
                        mean
                      </div>
                      <div className="text-right text-[var(--atlas-text)] tabular-nums">
                        {fmt(p.avg)}
              ...
              {p.semantic === "datetime" && (
                <>
                  <div className="text-[var(--atlas-text-subtle)]">first</div>
                  <div className="text-right text-[var(--atlas-text)] truncate">
                    {String(p.min ?? "—").slice(0, 10)}
                  </div>
                  <div className="text-[var(--atlas-text-subtle)]">last</div>
                  <div className="text-right text-[var(--atlas-text)] truncate">
              ...
              {p.semantic === "categorical" && p.topValues?.[0] && (
                <>
                  <div className="text-[var(--atlas-text-subtle)]">top</div>
                  <div className="text-right text-[var(--atlas-text)] truncate">
